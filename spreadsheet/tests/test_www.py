...
# See license.txt
"""www/spreadsheet.py — boot context + Vite manifest plumbing.
...
class AssetManifestResolution(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from spreadsheet.www import spreadsheet as www
    ...
    def test_resolves_hashed_paths_from_manifest(self):
        ...
        self.assertEqual(paths["js"], "/assets/spreadsheet/spreadsheet/index.ABCDEF.js")
        self.assertEqual(paths["css"], ["/assets/spreadsheet/spreadsheet/index.GHIJKL.css"])
    ...
    def test_falls_back_to_legacy_filenames_when_manifest_is_missing(self):
        ...
        paths = www._asset_paths()
        self.assertEqual(paths["js"], "/assets/spreadsheet/spreadsheet/index.js")
        self.assertEqual(paths["css"], ["/assets/spreadsheet/spreadsheet/index.css"])
...
class BootContext(unittest.TestCase):
    """get_context populates the keys spreadsheet.html injects into
    ...
    def test_sentry_dsn_picked_up_from_site_config(self):
        self.frappe.conf.get.side_effect = lambda key, default=None: {
            "spreadsheet_sentry_dsn": "https://k@o.ingest.sentry.io/p",
            "developer_mode":         0,
        }.get(key, default)
        from spreadsheet.www import spreadsheet as www
